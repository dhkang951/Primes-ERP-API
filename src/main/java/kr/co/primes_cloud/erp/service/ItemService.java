package kr.co.primes_cloud.erp.service;
...
public class ItemService extends BaseService {
    ...
    //품명품번규격 중복 체크
    public boolean checkDuplicate(ItemSearchRequest searchRequest) {
        // 품명, 품번, 규격 중 하나라도 입력되어야 함
        String itemName = searchRequest.getItemName();
        String itemNumber = searchRequest.getItemNumber();
        String itemSpec = searchRequest.getItemSpec();
        
        if((itemName == null || itemName.trim().isEmpty()) 
                && (itemNumber == null || itemNumber.trim().isEmpty()) 
                && (itemSpec == null || itemSpec.trim().isEmpty())) {
            throw new EntityNotFoundException(ErrorCode.ENTITY_NOT_FOUND_ERROR, String.format("품명, 품번, 규격 중 하나 이상을 입력해주세요."));
        }
        
        // 입력된 필드만으로 중복체크 수행
        List<ItemSearchResponse> isDuplicate = itemRepository.findDuplicatesByItemNameOrItemNumberOrItemSpec(itemName, itemNumber, itemSpec);
